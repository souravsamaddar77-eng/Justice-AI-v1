...
for (const filename of ['digital.pdf', 'scanned.pdf', 'corrupt.pdf']) {
  ...
  const response = await fetch(`${origin}/api/analyze-document`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({text:'', filename, fileBase64:bytes.toString('base64'), mode:'live', consent:{providers:config.providers.map(p=>p.id),ocr:true}}), signal:AbortSignal.timeout(60000) });
