...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (body.mode === "demo") return Response.json({ ...mockDraft(body), source: "mock" }, { headers: { "Cache-Control": "no-store" } });
    requireAIConsent(body);
    const prompt = `Document: ${body.documentType}\nClient: ${body.clientName}\nMatter: ${body.issue}\nDate: ${body.date}\nAdditional facts: ${body.additionalNotes || "None"}`;
    const input = { primary: "nemotron" as const, system: SYSTEM, messages: [{ role: "user" as const, content: prompt }], maxTokens: 2048, signal: req.signal };
