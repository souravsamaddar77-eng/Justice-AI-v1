import { AIError, aiErrorResponse, generateAI, requireAIConfiguration, languageInstruction } from "@/lib/ai";
...
export async function POST(req: Request) {
  ...
  try {
    const language = languageInstruction(body.language, true);
    ...
    if (body.fileBase64) {
      ...
      const extracted = await extractDocument({ bytes, filename: body.filename, signal: controller.signal, allowOCR: true });
    ...
    const result = await generateAI({ primary: "gemini", system: `${SYSTEM}\n${language}`, messages: [{ role: "user", content: text }], json: true, signal: controller.signal, validate: raw => { parseAnalysis(raw, text); } });
